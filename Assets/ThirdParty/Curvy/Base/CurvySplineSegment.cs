...
#region TR ADD #1
    // by anemos. 2013.12.19
    public int              mSegmentGranularity             = 0;
    private static float    APPROXIMATION_POINT_DISTANCE    = 0.1f;

    public void UpdateSegmentStepSize()
    {
        mStepSize = 1f / mSegmentGranularity;
    }
#endregion
    ...
    public bool SnapToFitSplineLength(float newSplineLength, float stepSize)
    {
        ...
        if (newSplineLength > curLength) {
            ...
            while (Spline.Length < newSplineLength) {
                ...
                if (loops == 0) {
                    YPLog.LogError("CurvySplineSegment.SnapToFitSplineLength exceeds 30000 loops, considering this a dead loop! This shouldn't happen, please report this as an error!");
                    return false;
                }
            }
        }
...
#region TR ADD #4
            // by anemos. 2013.12.19            
            // set up position of approximation point with same distance
            // to moving smooth through bezier curve
            if (Spline.Interpolation == CurvyInterpolation.Bezier)
            {
                // length of newly added segment is zero at first time 
                // cause of segment refresh order (approximation -> tangent -> orientation -> length)
                // so use spline's granularity once.
                // after all, use segment length to calculate mSegmentGranularity
                mSegmentIndex = Spline.Segments.IndexOf(this);
                if (0 != Length)
                    mSegmentGranularity = (int)(Length / APPROXIMATION_POINT_DISTANCE);
                
                mStepSize = 1f / mSegmentGranularity;
                Approximation = new Vector3[mSegmentGranularity + 1];
                ApproximationUp = new Vector3[mSegmentGranularity + 1];
                ApproximationT = new Vector3[mSegmentGranularity + 1];
                Approximation[0] = Position;
                Approximation[mSegmentGranularity] = NextTransform.position;
...
#region TR ADD #6 
            if (Spline.Interpolation == CurvyInterpolation.Bezier)
            {
                ...
                if (SmoothEdgeTangent)
                {
                    if (!IsFirstSegment)
                    {
                        PreviousSegment.ApproximationT[PreviousSegment.mSegmentGranularity] = Vector3.Lerp(PreviousSegment.ApproximationT[PreviousSegment.mSegmentGranularity - 1], ApproximationT[0], 0.5f);
                        ApproximationT[0] = PreviousSegment.ApproximationT[PreviousSegment.mSegmentGranularity];
                    }
                }
                if (IsLastSegment && NextSegment && Spline[0].SmoothEdgeTangent)
                { // closed spline
                    ApproximationT[mSegmentGranularity] = Vector3.Lerp(ApproximationT[mSegmentGranularity - 1], Spline[0].ApproximationT[0], 0.5f);
                    Spline[0].ApproximationT[0] = ApproximationT[mSegmentGranularity];
                }
            }
            else
            {
...
#region TR ADD #7
    ...
    {
        int granularity = Spline.Granularity;
#region TR ADD #8
        if (Spline.Interpolation == CurvyInterpolation.Bezier)
            granularity = mSegmentGranularity;
#endregion
        // Setup swirling
        float swirlangle=0;
        switch (Spline.Swirl) {
            case CurvyOrientationSwirl.Segment:
                swirlangle = Spline.SwirlTurns * 360 / (float)granularity;
                break;
            case CurvyOrientationSwirl.Spline:
                swirlangle = (Spline.SwirlTurns * 360 / Spline.Count) / (float)granularity;
                break;
        }
        ...
        if (Spline.Orientation == CurvyOrientation.Tangent) {
            ...
            lastUpVector = ApproximationUp[granularity];
        }
