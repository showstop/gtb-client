...
namespace Curvy.Utils
{
    ...
    public class MeshHelper
    {
        ...
        static Edge getNextEdge(int curVI, ref List<Edge> pool, out int nextVI)
        {
            ...
            nextVI = -1;
            YPLog.LogError("Curvy Mesh Builder: Open Edge Loop detected! Please check your StartMesh!");
            return null;
        }
        ...
        public static Mesh CreateSplineMesh(CurvySpline spline, int ignoreAxis, bool close)
        {
            ...
            float[] textureUs = spline.GetTextureU();

            if (spline.Closed)
                System.Array.Resize<Vector3>(ref verts, verts.Length - 1);

            mesh = buildSplineMesh(verts, ignoreAxis, !close);

            Vector2[] UV = new Vector2[textureUs.Length];
            for (int i = 0; i < textureUs.Length; ++i)
            {
                UV[i] = new Vector2(textureUs[i], mesh.uv[i].y);
            }
            mesh.uv = null;
            mesh.uv = UV;
            return mesh;
