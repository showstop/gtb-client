...
#region TR ADD #1
    public bool     SplitLane                   = false;
    public int      StartLaneNO                 = 1;
    public int      LaneCount                   = Constants.MAX_LANE_NO;
    public float    LaneWidth                   = Constants.DEFAULT_LANE_WIDTH;
    public float    LaneOffsetY                 = 0f;

    public CurvySplineSegment GetFirstSegment()
    {
    ...
    public CurvySplineSegment GetLastSegment()
    {
        CurvySplineSegment ret = null;
        foreach (CurvySplineSegment seg in mControlPoints)
        {
    ...
    public void UpdateSegmentStepSize()
    {
        ...
        {
            seg.UpdateSegmentStepSize();
        }
    }

    public float GetLaneOffsetX(int aLaneNO)
    {
        if (LaneCount == 1)
            return 0f;

        bool odd = ((LaneCount % 2) == 0) ? false : true;
        if (odd)
        {
            int midLaneNO = StartLaneNO + (LaneCount / 2);
            return LaneWidth * (aLaneNO - midLaneNO);
        }
        else
        {
            float startLaneOffset = (LaneWidth * -0.5f) + ((LaneCount / 2 - 1) * -LaneWidth);
            return startLaneOffset + ((aLaneNO - StartLaneNO) * LaneWidth);
        }
    }

    public float GetLaneTotalWidth()
    {
        ...
        if (!Application.isPlaying || AutoRefresh) {
            ...
            for (int i = 0; i < mControlPoints.Count; i++) {
                if (mControlPoints[i]!=null && (mControlPoints[i]._PositionHasChanged() || mControlPoints[i]._RotationHasChanged())) {
                    ...
                    //break;
#region TR ADD #2   
                    List<CurvyConnection> cons = mControlPoints[i].GetAllConnections(0, "");
                    foreach (CurvyConnection con in cons)
                    {
                        con.RefreshSplines();
                    }                    
#endregion 
...
#region TR ADD #3 fix null segment reference - 1
        // by anemos. 2014.01.02
        // return value of TFToSegment is null when try to make mesh of split spline sometimes.
        // maybe try to reference connected spline before refreshing (Count == 0)
        // because too many code is using TFToSegment function, i can't fix every line.
        // please fix it when other error is occurred.
        if (null == seg)
            return Vector3.zero;
#endregion
    ...
    public override Vector3 InterpolateFast(float tf)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
#region TR ADD #3 fix null segment reference - 2
        if (null == seg)
            return Vector3.zero;
#endregion
    ...
    public override Vector3 InterpolateUserValue(float tf, int index)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
#region TR ADD #3 fix null segment reference - 3
        if (null == seg)
            return Vector3.zero;
#endregion
    ...
    public override Quaternion GetOrientationFast(float tf)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
#region TR ADD #3 fix null segment reference - 4
        if (null == seg)
...
#region TR MODIFY #1
    // by anemos. 2013.12.20    
    int GetApproximationPointCount()
    {
        int ret = Count * Granularity;
        if (Interpolation == CurvyInterpolation.Bezier)
        {
            ret = 0;
            for (int si = 0; si < Count; ++si)
            {
                ret += this[si].mSegmentGranularity;
            }
        }
    ...
    {
        Vector3[] apps = new Vector3[GetApproximationPointCount() + 1];
        int idx = 0;
        for (int si = 0; si < Count; si++)
        {
    ...
    {
        float[] textureus = new float[Count];
        for (int i = 0; i < Count; ++i)
        {
            textureus[i] = this[i].TextureU;
        }
        return textureus;
    }
...
#region TR MODIFY #2
            // by anemos. 2013.12.20
            int totalGranularity = Count * Granularity;
            int lastGranularity = Granularity;
            if (Interpolation == CurvyInterpolation.Bezier)
            {
                totalGranularity = GetApproximationPointCount();
                lastGranularity = this[Count - 1].mSegmentGranularity;
            }
#endregion

            if (!Closed && SetControlPointRotation && Orientation == CurvyOrientation.Tangent && this[Count - 1].NextControlPoint)
                this[Count - 1].NextControlPoint.Transform.up = this[Count - 1].ApproximationUp[lastGranularity];

            // Make PTFrames seamless
            if (Closed && Orientation == CurvyOrientation.Tangent) {
                float smoothingAngle = AngleSigned(this[Count - 1].ApproximationUp[lastGranularity - 1], this[0].ApproximationUp[0], this[0].ApproximationT[0]) / totalGranularity;
                ...
                // last==first
                this[Count - 1].ApproximationUp[lastGranularity] = this[0].ApproximationUp[0];
            }
        }
    }
