...
#region TR ADD #1
        if (Target.mStartConnectionPoint != Vector3.zero)
            GUILayout.Label("Start Connection Point : " + Target.mStartConnectionPoint);

        if (Target.mEndConnectionPoint != Vector3.zero)
            GUILayout.Label("End Connection Point : " + Target.mEndConnectionPoint);
#endregion
        ...
        if (GUILayout.Button(new GUIContent(mTexDetach,"Clone to an individual GameObject"),GUILayout.ExpandWidth(false))) {
            Unwrapping.GenerateSecondaryUVSet(Target.Mesh);
