...
#region TR ADD #1
    private float   mLaneOffset             = 0f;
    public Vector3  mStartConnectionPoint   = Vector3.zero;
    public Vector3  mEndConnectionPoint     = Vector3.zero;

    void ApplyOffsetToVertices(int aVertIndex, CurvySpline aSpline)
    {
        Vector3 vertexOffset = mVerts[aVertIndex + 1] - mVerts[aVertIndex];

        CurvySpline cs = (CurvySpline)Spline;
        bool odd = ((cs.LaneCount % 2) == 0) ? false : true;
        if (odd)
        {
            int midLaneNO = cs.StartLaneNO + (cs.LaneCount / 2);
            mLaneOffset = aSpline.StartLaneNO - midLaneNO;
        }
        else
        {
            mLaneOffset = -0.5f + ((cs.LaneCount / 2 - 1) * -1f) + (aSpline.StartLaneNO - cs.StartLaneNO);            
        }

        vertexOffset *= mLaneOffset;

        mVerts[aVertIndex] += vertexOffset;
        mVerts[aVertIndex + 1] += vertexOffset;
        if (aSpline.LaneCount > 1)        
        {
            Vector3 multiSplitOffset = (mVerts[aVertIndex + 1] - mVerts[aVertIndex]) * (aSpline.LaneCount - 1);
            mVerts[aVertIndex + 1] += multiSplitOffset;
        }

        if (0 == aVertIndex)
        {            
            mStartConnectionPoint = (mVerts[aVertIndex] + mVerts[aVertIndex + 1]) / 2f;
            CurvySplineSegment firstSegment = aSpline.GetFirstSegment();
            if (null != firstSegment && firstSegment.gameObject.transform.position != mStartConnectionPoint)            
                firstSegment.gameObject.transform.position = mStartConnectionPoint;
        }
        else
        {
            mEndConnectionPoint = (mVerts[aVertIndex] + mVerts[aVertIndex + 1]) / 2f;            
            CurvySplineSegment lastSegment = aSpline.GetLastSegment();
            if (null != lastSegment && lastSegment.gameObject.transform.position != mEndConnectionPoint)
                lastSegment.gameObject.transform.position = mEndConnectionPoint;
        }
    }
#endregion
...
#region TR ADD #2
            /*
             * by anemos. 2013.12.27
             * currently only work with unclosed mesh            
             */
            CurvySpline cs = (CurvySpline)Spline;
            if (cs.SplitLane && CapShape == MeshCapShape.Line)
            {
                ...
                for (int seg = 0; seg < segSteps; seg++)
                {
                    ...
                    {   
                        // check connection
                        CurvySplineSegment checkSegment = null;                        
                        if (seg == 0)
                        {
                            checkSegment = cs.GetFirstSegment();
                        }
                        else if (seg == SegmentSteps - 1 || seg == SegmentSteps - 2)
                        {
                            checkSegment = cs.GetLastSegment();
                        }

                        bool existConnection = false;
                        CurvyConnection con = null;
                        if (null != checkSegment)
                        {
                            con = checkSegment.ConnectionAny;
                            if (null != con)
                        ...
                        if (existConnection)
                        {
                            ...
                            CurvySpline oriSpline = (CurvySpline)Spline;
                            if (con.Owner.Spline != oriSpline)
                                Spline = con.Owner.Spline;
                            else
                                Spline = con.Other.Spline;

                            float tf = (seg == 0) ? 1f : 0f;
                            Vector3 scale = getScale(tf);
                            CurvyMeshSegmentInfo conSegmentInfo = new CurvyMeshSegmentInfo(this, tf, scale);
                            mVerts[vIndex + v] = conSegmentInfo.Matrix.MultiplyPoint3x4(StartMeshInfo.Vertices[v]);                            

                            if (v == 1)
                                ApplyOffsetToVertices(vIndex, oriSpline);
                        ...
                        else
                        {
                            ...
                            if (v == 1)
                            {
                                ApplyOffsetToVertices(vIndex, (CurvySpline)Spline);
                                /*
                                cs = (CurvySpline)Spline;
                                if (cs.LaneCount > 1)
                                {
                                    Vector3 multiSplitOffset = (mVerts[vIndex + 1] - mVerts[vIndex]) * (cs.LaneCount - 1);
                                    mVerts[vIndex + 1] += multiSplitOffset;
                                }
                                */ 
                            }
                        }
