...
#region TEXT_ANIMATION
    private int m_charsPerSecond = 40;
    private int m_OffsetTextAnimation = 0;
    private float m_NextChar = 0f;

    private string m_textAnimation = "";          //< 애니메이션할 텍스트
    public string TextAnimation
    {
        set
        {
            supportEncoding = false;
            symbolStyle = NGUIText.SymbolStyle.None;

            m_textAnimation = value;
            m_OffsetTextAnimation = 0;

            StopCoroutine("DelayTextAnimation");
            StartCoroutine(DelayTextAnimation());
        }
    }

    System.Collections.IEnumerator DelayTextAnimation()
    {
        while (m_OffsetTextAnimation < m_textAnimation.Length)
        {
            if (m_NextChar <= RealTime.time)
            {
                m_charsPerSecond = Mathf.Max(1, m_charsPerSecond);

                float delay = 1f / m_charsPerSecond;
                char c = m_textAnimation[m_OffsetTextAnimation];
                if (c == '.' || c == '\n' || c == '!' || c == '?') delay *= 4f;

                m_NextChar = RealTime.time + delay;
                text = m_textAnimation.Substring(0, ++m_OffsetTextAnimation);
            }
            yield return null;
        }
