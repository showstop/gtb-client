...
public class UIGrid : UIWidgetContainer
{
	...
	public bool useCullContent = false;
...
#region TR ADD #1
    ...
    protected virtual void OnMove(UIPanel panel) { CullContent(); }

    public void CullContent()
    {
		if(false == useCullContent)	return;
        ...
        if (UIScrollView.Movement.Horizontal == mDrag.movement)
        {
            ...
            for (int i = 0; i < transform.childCount; ++i)
            {
                ...
                if (!UICamera.IsPressed(t.gameObject))
                    NGUITools.SetActive(t.gameObject, (distance > min && distance < max));
            }
        }
        else if (UIScrollView.Movement.Vertical == mDrag.movement)
        {
