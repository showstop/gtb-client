...
static public class NGUITools
{
	...
	{
		if ( false == TRGlobalData.instance.PlaySoundEffect )
			return null;
	...
	{
#if UNITY_FLASH
		YPLog.LogError("WWW is not yet implemented in Flash");
		return null;
#else
		WWW www = null;
		try { www = new WWW(url); }
		catch (System.Exception ex) { YPLog.LogError(ex.Message); }
		return www;
#endif
	}
	...
	static public WWW OpenURL (string url, WWWForm form)
	{
		...
		catch (System.Exception ex) { YPLog.LogError(ex != null ? ex.Message : "<null>"); }
		return www;
#endif
	}
