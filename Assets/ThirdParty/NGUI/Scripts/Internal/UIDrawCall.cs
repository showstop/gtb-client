...
public class UIDrawCall : MonoBehaviour
{
	...
	void CreateMaterial ()
	{
		...
		if (mMaterial != null)
		{
			...
			{
				YPLog.LogError(shaderName + " doesn't have a clipped shader version for " + mClipping);
	...
	void UpdateMaterials ()
	{
		...
		{
#if UNITY_EDITOR
			YPLog.LogError("Hmm... This point got hit!");
#endif
	...
	public void Set (BetterList<Vector3> verts, BetterList<Vector3> norms, BetterList<Vector4> tans, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			else
			{
				...
				YPLog.LogError("Too many vertices on one panel: " + verts.size);
			}
		...
		{
			if (mFilter.mesh != null) mFilter.mesh.Clear();
			YPLog.LogError("UIWidgets must fill the buffer with 4 vertices per quad. Found " + count);
		}
	}
