...
// 				range.xMax = Mathf.Max(range.yMax, m_AniCurve[n].time);
 	...
 	public override int GetAnimationState()
	{
		if (enabled == false || IsActive(gameObject) == false)
			return -1;
// 		YPLog.Log(m_fStartTime);
// 		YPLog.Log(IsEndAnimation());
...
// 	}
	...
	void Awake()
	{
// 		YPLog.Log("NcCurveAnimation.Awake " + transform.parent.name);
	}

	void Start()
	{
//		YPLog.Log("NcCurveAnimation.Start " + transform.parent.name);
	...
	void LateUpdate()
	{
// 		YPLog.Log("NcCurveAnimation.Update " + transform.parent.name);
		if (m_fStartTime == 0)
			return;
...
// 					{
// 						YPLog.Log(GetHelpString("DUPLICATE", string.Format("CurveIndex [{0}], TypeValue[{1}]", nIndex.ToString(), m_CurveInfoList[nIndex].GetValueName(n))));
// 						bDup = true;
// 					}
// 			}
// 		}
