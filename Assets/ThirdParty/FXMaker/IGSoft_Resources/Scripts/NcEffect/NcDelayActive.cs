...
// 		YPLog.Log("Awake" + m_fDelayTime + gameObject.name);
		if (m_fDelayTime != 0)
		{
	...
	{
		return;	// not use
// 		YPLog.Log("InitNcDelayActive " + m_fStartedTime + gameObject.name);
...
//  			YPLog.Log("InitNcDelayActive " + fInvokeTime + gameObject.name);
...
// 				YPLog.Log(m_fParentDelayTime);
	...
	{
// 		YPLog.Log("CancelDelayActive " + name);
		m_fStartedTime = 0;
	...
	void OnStartActive()
	{
//   		YPLog.Log("OnStartActive" + m_fStartedTime + gameObject.name);
		if (0 < m_fStartedTime)
			return;
