...
public class NgSerialized
{
	...
	public static void SetPropertyValue(SerializedObject serObj, string propertyName, object value, bool bUpdate)
	{
		...
		if (sp == null)
		{
			YPLog.LogError("SetPropertyValue error - " + propertyName);
			return;
		}
...
// 			default: YPLog.Log("Undefile propertyType - GetPropertyValue " + sp.name);		break;
		}
		return value;
	}
...
// 			default: YPLog.Log("Undefile propertyType - SetPropertyValue " + sp.name); break;
		}
	}
	...
	public static Component AddComponent(GameObject tarObject, Component srcCom)
	{
        ...
        if (srcCom.ToString().Contains("WorldParticleCollider"))
			return UnityEngineInternal.APIUpdaterRuntimeServices.AddComponent(tarObject, "Assets/ThirdParty/FXMaker/IGSoft_Tools/CommonLib/GlobalScript/NgSerialized.cs (190,11)", "WorldParticleCollider");
...
// 				YPLog.Log((bool)GetPropertyValue(sysSo, "ShapeModule.enabled"));
// 				YPLog.Log((int)GetPropertyValue(sysSo, "ShapeModule.type"));
