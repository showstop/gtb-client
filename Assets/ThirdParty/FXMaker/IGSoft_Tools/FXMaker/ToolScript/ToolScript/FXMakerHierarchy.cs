...
public class FXMakerHierarchy : MonoBehaviour
{
	...
	void Awake()
	{
		...
		if (m_DisableTexture == null)
			YPLog.LogError("m_DisableTexture is null !!! - Disanble.png");
		if (m_WarringTexture == null)
			YPLog.LogError("m_WarringTexture is null !!! - Warring.png");
	}
...
// 							YPLog.LogWarning("Missage script !!! - GameObject : " + drawTrans.name);
...
// 		YPLog.Log("UpdateMeshList");
		...
		{
// 			YPLog.Log(NgSerialized.IsMesh(com, true));
...
// 			return;
	...
	public void OnCreateInstanceEffect(GameObject instanceObj, bool bReset, GameObject parentInstanceObj)
	{
		...
		if (instanceObj != null)
		{
			...
			// BoundsBox, Wireframe
// 			YPLog.Log(parentIndexCom.m_bSelected);
			if (bSel)
